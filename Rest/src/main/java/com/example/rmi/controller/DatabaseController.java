...
public class DatabaseController {
    ...
    public ResponseEntity<?> addColumn(@Valid @RequestBody ColumnRequest columnRequest) throws RemoteException {
        remoteDB.addColumn(columnRequest.getTableIndex(), columnRequest.getName(), columnRequest.getColumnType());
    ...
    @Operation(summary = "Sort by column", description = "Sorts table by column")
    @ApiResponse(responseCode = "200", description = "Table sorted successfully",
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = Table.class)))
    public ResponseEntity<?> sortByColumn(@Valid int tableIndex,@Valid int columnIndex) throws RemoteException {
        remoteDB.sortByColumn(tableIndex,columnIndex);
        Table tableDetails = new Table(remoteDB.getTablesData().get(tableIndex).name);
        for (int i = 0; i < remoteDB.getColumns(tableIndex).size(); i++) {
            tableDetails.addColumn(remoteDB.getColumns(tableIndex).get(i));
        }
