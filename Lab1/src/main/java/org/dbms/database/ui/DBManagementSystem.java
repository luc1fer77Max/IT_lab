...
public class DBManagementSystem {
    ...
    public static DBManagementSystem getInstance(){
        if (instance == null){
            ...
            instance.sortByColumn = new JMenuItem("Sort");
    ...
    public static void main(String[] args) {
        ...
        instance.columnMenu.add(instance.sortByColumn);
        ...
        instance.sortByColumn.addActionListener(e -> {
            int selectedTab = instance.tabbedPane.getSelectedIndex();
            JPanel tablePanel = (JPanel) instance.tabbedPane.getComponentAt(selectedTab);
            JScrollPane scrollPane = (JScrollPane) tablePanel.getComponent(0);
            JTable table = (JTable) scrollPane.getViewport().getView();
            ...
            if (selectedTab != -1 && selectedColumn != -1) {
                DBManager.getInstance().sortByColumn(selectedTab, selectedColumn);
                for (int i1 = tableModel.getRowCount()-1; i1 >= 0 ; i1--) {
                    tableModel.removeRow(i1);
                }
                List<Row> rows;
                int columnSize;
                rows = DBManager.database.tables.get(selectedTab).rows;
                columnSize = DBManager.database.tables.get(selectedTab).columns.size();
                ...
                JOptionPane.showMessageDialog(instance.frame, "Sorted!");
            }
        });
    }
